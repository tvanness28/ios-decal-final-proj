...
class SearchTableViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        addButton.setTitle("Add \(items[SearchTableViewController.searchIndex])", for: UIControlState.normal)
    ...
    func updateData() {
        ...
        switch SearchTableViewController.searchIndex {
        case 0:
            let fetchRequest:NSFetchRequest<Plan> = Plan.fetchRequest()
            do {
                plans = try managedContext.fetch(fetchRequest)
            } catch {
                fatalError("Failed to fetch \(items[SearchTableViewController.searchIndex]): \(error)")
            }
            break
        case 1:
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case 2:
            if (foods.count > 0) {
                let food = foods[indexPath.row]
                self.delegate?.searchViewControllerResponse(food: food)
